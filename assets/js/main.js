(function () {
  ...
  function setupTitleAnimation() {
    ...
    words.forEach((word, i) => {
      setTimeout(() => word.classList.add('in'), 200 + i * 70);
    });
  }
  ...
  /**
   * Card spotlight: tracks cursor position and exposes it via CSS vars (--mx, --my)
   * so the CSS ::before radial-gradient follows the pointer.
   * Applied to all interactive cards. No 3D rotation, no transform —
   * the visual change happens purely via the CSS spotlight + border-glow on :hover.
   */
  function setupCardSpotlight() {
    if (reduceMotion) return;
    if (window.matchMedia('(pointer: coarse)').matches) return;

    const cards = document.querySelectorAll('.card, .tool-group');
